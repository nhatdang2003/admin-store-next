...
export function PromotionForm({
    ...
    const uploadBanner = async (file: File): Promise<string> => {
        try {
            ...
            // Lấy presigned URL
            const presignedData = await imageApi.getPresignedUrlPromotion(file.name);

            // Upload file lên Google Cloud
            await imageApi.uploadImage(presignedData.signedUrl, file);
    ...
    const handleSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            ...
            // Nếu có file banner mới được chọn, upload lên cloud
            if (values.imageUrl) {
                imageUrl = await uploadBanner(values.imageUrl);
            }
            ...
            toast({
                variant: "destructive",
                title: "Lỗi upload banner",
                description: "Không thể upload ảnh banner. Vui lòng thử lại.",
            });
        }
    };
        ...
        <Form {...form}>
            ...
            >
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel>Banner khuyến mãi</FormLabel>
                            <FormControl>
                                <ImageUpload
                                    value={field.value || ""}
                                    onChange={(file) => field.onChange(file)}
                                    onRemove={() => field.onChange(null)}
                                    preview={promotion?.imageUrl}
                ...
                <div className="flex items-center justify-end gap-4">
                    ...
                    <AlertDialog>
                        <AlertDialogTrigger asChild>
                            <Button type="button" disabled={isLoading || uploading}>
                                {uploading
                                    ? "Đang upload..."
                                    : promotion
                                        ? "Cập nhật"
